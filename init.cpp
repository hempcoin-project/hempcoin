...
// Copyright (c) 2017 The Hempcoin developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#if defined(HAVE_CONFIG_H)
#include "config/hempcoin-config.h"
#endif
...
// the DetectShutdownThread(), which interTHCts the main thread group.
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "hempcoin.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "hempcoind.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 27577, 27777));
    ...
    strUsage += HelpMessageOpt("-salvagewallet", _("Attempt to recover private keys from a corTHCt wallet.dat") + " " + _("on startup"));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, hempcoin, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Hempcoin specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:27577"));
...
//    strUsage += "  -anonymizehempcoinamount=<n>     " + strprintf(_("Keep N THC anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 27777, 38843));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Hempcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Hempcoin is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Hempcoin Core is shutting down."));
...
#endif
    // Make sure only a single Hempcoin process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Hempcoin Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Hempcoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (!fDisableWallet) {
        ...
        if (filesystem::exists(GetDataDir() / strWalletFile)) {
            ...
            if (r == CDBEnv::RECOVER_OK) {
                string msg = strprintf(_("Warning: wallet.dat corTHCt, data salvaged!"
            ...
            if (r == CDBEnv::RECOVER_FAIL)
                return InitError(_("wallet.dat corTHCt, salvage failed"));
        }
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Hempcoin: load previous sessions sporks if we have them.
                ...
                // Hempcoin: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Hempcoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Hempcoin Core to complete") << "\n";
...
//    }
//
//    nAnonymizeHempcoinAmount = GetArg("-anonymizehempcoinamount", 0);
//    if (nAnonymizeHempcoinAmount > 999999) nAnonymizeHempcoinAmount = 999999;
//    if (nAnonymizeHempcoinAmount < 2) nAnonymizeHempcoinAmount = 2;
    ...
    LogPrintf("Anonymize Hempcoin Amount %d\n", nAnonymizeHempcoinAmount);
