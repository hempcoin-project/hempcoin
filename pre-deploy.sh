...
if [[ $HOST = "i686-w64-mingw32" ]]; then
  ARCHIVE_NAME="thc-windows-x86.zip"
    ...
    ARCHIVE_NAME="thc-linux-x64.tar.gz"
    ARCHIVE_CMD="tar -czf"
elif [[ $HOST = "x86_64-apple-darwin11" ]]; then
    ARCHIVE_NAME="thc-osx-x64.zip"
fi

if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find /home/travis/build/hempcoin-project/hempcoin/ -type f | grep -r -i *.dmg$ | xargs -i cp {} $OUTDIR/bin
else
